...
def replace_numbers_with_strings(matrix, file_path):
    ...
    for i in range(len(matrix)):
        if matrix[i] < len(lines):
            matrix[i] = lines[matrix[i]].strip()  # strip to remove newline characters
...
def get_worst_first_orders_index(target_path,original_order,t):
    min_order_index=-1
    min_method_count = 0
    summary_gen=0
    current_order=[]
    for filename in os.listdir(target_path):
        ...
        if os.path.isfile(file_path):
            with open(file_path, "r") as file:
                ...
                orders,string_conversion_time=replace_numbers_with_strings(current_order,original_order)
                order_max_inter_class_copy=copy.deepcopy(orders)
                order_summary_copy=copy.deepcopy(orders)
                method_summary=rank_orders.summarize_test_methods(order_summary_copy)
                                #
                current_interclass_combinations = rank_orders.find_interclass_pairs(order_max_inter_class_copy)
...
def sort_orders_based_on_coverage_and_worst_first(orders, t, method_summary, module, worst_first_index):
    ...
    parent_dir_name = "pairwise worst first absolute interclass"
    ...
    while orders:
        ...
        for idx, order in enumerate(orders):
            if first_flag == 0:
                max_order_index=worst_first_index
                first_flag=1
                break
    ...
    try:
        ...
        with open(input_csv, mode='r', newline='', encoding='utf-8') as file:
            ...
            for row in csv_reader:
                ...
                worst_first_index=get_worst_first_orders_index(target_path,original_order,t)
                print(worst_first_index)
                exit(1)
