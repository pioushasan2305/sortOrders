...
def sort_orders_based_on_coverage_and_best_first(orders, t, method_summary, module, best_first_index, github_slug):
    ...
    parent_dir_name = "pairwise best first relative interclass"
    ...
    first_flag=0
    while orders:
        max_cover = max_interclass_cover = 0
        max_order_index = -1
        ...
        for idx, order in enumerate(orders):
            ...
            if current_cover > max_cover:
                ...
                max_order_index = idx
            elif current_cover == max_cover and current_interclass_cover > max_interclass_cover:
                max_interclass_cover = current_interclass_cover
                max_order_index = idx
                tie_break_count += 1
            elif current_cover == max_cover and current_interclass_cover == max_interclass_cover:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        sys.exit(1)
    parent_dir_name = "pairwise best first relative inter-class OD"
    parent_dir_path = os.path.join(parent_dir_name)
