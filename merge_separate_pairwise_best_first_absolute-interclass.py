...
import string
import OD_detection
def sort_orders_based_on_coverage_and_best_first(orders, t, method_summary, module, best_first_index,instance_no, github_slug,first_idx):
    ...
    csv_file_path = os.path.join(parent_dir_name, f"{module}_order_statistics_{instance_no}.csv")
    module = module + "_" + str(instance_no)
    ...
    while orders:
        ...
        if max_order_index != -1:
            ...
            with open(csv_file_path, 'a', newline='') as file:
                writer = csv.writer(file)
                writer.writerow([file_count, time_taken_to_sort, tie_break_count, tie_of_tie_break_count,idx+first_idx])
...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        print("Usage: python script.py <path_to_csv_file> <Total No of instances> <Instance No>")
        sys.exit(1)
    ...
    with open(input_csv, mode='r', newline='', encoding='utf-8') as file:
        ...
        for row in csv_reader:
            ...
            distributed_orders,first_idx,last_idx=rank_orders.distribute_orders_evenly(orders, total_instances, instance_no)
            print(f"first_idx-{first_idx}")
            print(f"last_idx-{last_idx}")
            order_sorted_copy_best_first= copy.deepcopy(distributed_orders)
            order_max_inter_class_copy=copy.deepcopy(distributed_orders)
            ...
            sorted_orders_max_inter_class,total_time_taken_to_sort,sorted_orders_path =sort_orders_based_on_coverage_and_best_first(order_max_inter_class_copy, t ,method_summary ,module ,best_first_index,instance_no, github_slug,first_idx)
